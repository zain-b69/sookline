...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $img = $_POST['img'];
    $prix = $_POST['prix'];
    $stmt = $conn->prepare("INSERT INTO produits (nom, description, categorie, img, prix) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("ssssd", $nom, $desc, $categorie, $img, $prix);
    if ($stmt->execute()) {
        $success = true;
    } else {
...
<body>
 ...
 <div class="top-bar">
        ...
        <div><strong>Administration</strong> - Ajouter des Produits</div>
  ...
  <div class="dashboard-card">
    ...
    <form method="post">
      ...
      <input type="text" name="img" placeholder="Lien de l'image" required>
      <input type="number" name="prix" placeholder="Prix (DH)" min=1 required>
